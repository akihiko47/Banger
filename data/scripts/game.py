...
from data.scripts.pause import *
from data.scripts.meteor import *
from data.scripts.turret import *
...
class Game:
    def __init__(self):
        ...
        self.enemy_types_0 = (
            {'image': enemy_img_0, 'speed': random.uniform(1.5, 2.0), 'hp': 1, 'damage': 5, 'cost': 3},
            {'image': enemy_img_1, 'speed': random.uniform(4.0, 5.0), 'hp': 1, 'damage': 10, 'cost': 8},
            {'image': enemy_img_2, 'speed': random.uniform(3.0, 3.5), 'hp': 3, 'damage': 20, 'cost': 12})

        self.enemy_types_1 = (
            {'image': enemy_img_0, 'speed': random.uniform(1.5, 2.0), 'hp': 1, 'damage': 10, 'cost': 2},
            {'image': enemy_img_1, 'speed': random.uniform(4.0, 5.0), 'hp': 1, 'damage': 20, 'cost': 6},
            {'image': enemy_img_2, 'speed': random.uniform(3.0, 3.5), 'hp': 4, 'damage': 30, 'cost': 10})

        self.enemy_types_2 = (
            {'image': enemy_img_0, 'speed': random.uniform(1.5, 2.0), 'hp': 1, 'damage': 20, 'cost': 2},
            {'image': enemy_img_1, 'speed': random.uniform(4.0, 5.0), 'hp': 2, 'damage': 30, 'cost': 6},
            {'image': enemy_img_2, 'speed': random.uniform(3.0, 3.5), 'hp': 5, 'damage': 40, 'cost': 10})
    ...
    def game_menu(self):
        ...
        while menu:
            ...
            message_to_screen(screen, 'Difficulties:', YELLOW, med_font, 400, 270)
    ...
    def create_player(self, x, y):
        ...
        return Player(x, y,
                      self.player_speed,
                      hp,
                      f'data/images/player/cannon_{self.player_cannon_level}.png',
                      f'data/images/player/player_model_{self.player_body_level}.png')
    ...
    def death_screen(self):
        ...
        pygame.display.update()
        sleep(2)
        pygame.mixer.music.load('data/sounds/sad.mp3')
        pygame.mixer.music.set_volume(self.volume)
        pygame.mixer.music.play(-1)
        ...
        while death:
            ...
            """score message"""
            message_to_screen(screen, 'DEAD', RED, large_font, 400, 100)
            message_to_screen(screen, f'you survived {self.time} seconds', RED, med_font, 400, 200)
            if self.difficulty == 0:
                message_to_screen(screen, f'on easy difficulty', RED, med_font, 400, 250)
            if self.difficulty == 1:
                message_to_screen(screen, f'on medium difficulty', RED, med_font, 400, 250)
            if self.difficulty == 2:
                message_to_screen(screen, f'on hard difficulty', RED, med_font, 400, 250)
    ...
    def spawn_wave(self):
        ...
        if self.difficulty == 0:
            try:
                ...
                self.add_enemy_to_group(self.enemy_types_0, '33' * self.wave)
        elif self.difficulty == 1:
            try:
                self.add_enemy_to_group(self.enemy_types_1, self.waves_1[self.wave])
            except IndexError:
                self.add_enemy_to_group(self.enemy_types_1, '33' * self.wave)
        else:
            try:
    ...
    def game(self):
        ...
        sleep(1)
        if self.difficulty == 0:
            pygame.mixer.music.load('data/sounds/easy_music.mp3')
        elif self.difficulty == 1:
            pygame.mixer.music.load('data/sounds/medium_music.mp3')
        else:
            pygame.mixer.music.load('data/sounds/hard_music.mp3')
        pygame.mixer.music.set_volume(self.volume)
        pygame.mixer.music.play(-1)
        ...
        while in_game:
            ...
            message_to_screen(screen, 'HP : ', DEEP_BLUE, small_font, 530+shake_dist, 630+shake_dist)
            ...
            message_to_screen(screen, f'cost: {self.med_cost}', YELLOW, small_font, 95+shake_dist, 715+shake_dist)

            meteor_big_button.draw('', small_font, mouse)
            screen.blit(meteor_big_img, (159+shake_dist, 707+shake_dist))
            message_to_screen(screen, f'cost: {self.big_cost}', YELLOW, small_font, 200+shake_dist, 715+shake_dist)

            """turret"""
            turret_button.draw('', small_font, mouse)
            screen.blit(turret_full_img, (364+shake_dist, 685+shake_dist))
            message_to_screen(screen, f'cost: {self.turret_cost}', YELLOW, small_font, 384+shake_dist, 678+shake_dist)
            ...
            """data on screen"""
            message_to_screen(screen, f'Points: {self.money}', YELLOW, med_font, 150+shake_dist, 680+shake_dist)

            pygame.draw.rect(screen, ORANGE, (300+shake_dist, -15+shake_dist, 200, 110), 5, 15, 0, 0, 15)
            message_to_screen(screen, f'Time: {self.time}', YELLOW, med_font, 400+shake_dist, 30+shake_dist)
            message_to_screen(screen, f'Wave {self.wave}', YELLOW, small_font, 400+shake_dist, 70+shake_dist)
